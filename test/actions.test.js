...
import oadaCacheOvermind from '../src'
...
describe('Actions', () => {
  ...
  it('should connect using a token', () => {
    const oadaConfig = oadaCacheOvermind('oada') //Mount at namespace 'oada' in state
    const {state, actions, effects} = createOvermindMock(namespaced({oada: oadaConfig}), {oada: oadaConfig.effects}) //Use real effects
  ...
  xit('should set test data in state', async () => {
